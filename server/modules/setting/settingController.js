...
settingController.GetSettingAll = async (req, res, next) => {
  try {
    let { page, size, populate, selectQuery, searchQuery, sortQuery } =
      otherHelper.parseFilters(req, 10, false);
    let admin_role_id = await getSetting("roles", "admin", "admin_id");
    let super_admin_role_id = await getSetting(
      "roles",
      "super_admin",
      "super_admin_id"
    );
    if (
      req.user.roles.includes(admin_role_id) &&
      !req.user.roles.includes(super_admin_role_id)
    ) {
      searchQuery = { admin: true, ...searchQuery };
    }
    if (req.query.find_type && req.query.find_type != "all") {
      searchQuery = {
        type: { $regex: req.query.find_type, $options: "i" },
        ...searchQuery,
      };
    }
    ...
    if (
      req.query.find_sub_type &&
      req.query.find_type != "all" &&
      req.query.find_sub_type != "all"
    ) {
      searchQuery = {
        sub_type: { $regex: req.query.find_sub_type, $options: "i" },
        ...searchQuery,
      };
    }
    ...
    let setting = await otherHelper.getQuerySendResponse(
      settingSch,
      page,
      size,
      sortQuery,
...
settingController.GetSettingType = async (req, res, next) => {
  try {
    let { page, size, populate, selectQuery, searchQuery, sortQuery } =
      otherHelper.parseFilters(req, 10, null);
    ...
    if (req.query.find_value) {
      searchQuery = {
        value: { $regex: req.query.find_value, $options: "i" },
        ...searchQuery,
      };
    }
...
settingController.SaveSetting = async (req, res, next) => {
  try {
    ...
    if (data._id) {
      ...
      let updated = await settingSch.findByIdAndUpdate(data._id, {
        $set: data,
      });
      return otherHelper.sendResponse(
        res,
  ...
  try {
    ...
    await Promise.all(
      Object.keys(data).map(async (each) => {
        let edited = await settingSch.findOneAndUpdate(
          { key: each },
          {
            $set: {
              value: req.body[each].value,
              updated_at: Date.now(),
        ...
        if (edited) {
          ...
          const newSetting = new settingSch({
            title: each,
            key: each,
            value: req.body[each].value,
            updated_at: Date.now(),
...
settingController.DeleteSettings = async (req, res, next) => {
  try {
    ...
    const temp = await settingSch.findOne({
      _id: settingId,
      is_removable: true,
    });
    if (temp && temp._id) {
      if (
        ...
        if (temp.added_by == req.user.id) {
          ...
          return otherHelper.sendResponse(
            ...
            null,
            error,
            "Cannot delete this setting",
            null
          );
        }
      }
      const del = await settingSch.findByIdAndUpdate(settingId, {
        $set: { is_deleted: true },
      });
      return otherHelper.sendResponse(
        res,
